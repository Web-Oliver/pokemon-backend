...
export default class IcrTextDistributionService {
    ...
    async distributeOcrText(batchId, ocrResult = null) {
        try {
            ...
            // Keep stitched label status as 'ocr_complete' - distribution doesn't change processing stage
            // Just update the distribution metrics
    ...
    async distributeOcrTextByHashes(imageHashes, ocrResult = null) {
        try {
            ...
            // CRITICAL FIX: Process scans in the exact order of imageHashes to preserve text distribution
            for (let hashIndex = 0; hashIndex < imageHashes.length; hashIndex++) {
                const imageHash = imageHashes[hashIndex];
                ...
                const distributedText = distributedTexts[hashIndex] || '';
