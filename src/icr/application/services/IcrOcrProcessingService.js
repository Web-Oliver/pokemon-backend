...
export class IcrOcrProcessingService {
    ...
    async processOcr(stitchedImagePath) {
        ...
        return OperationManager.executeFileOperation(
            ...
            async () => {
                ...
                // Update individual GradedCardScans status to reflect OCR completion
                // Find scans that are part of this stitched label
                const scanHashes = stitchedLabel.labelHashes || [];
                if (scanHashes.length > 0) {
                    await this.gradedCardScanRepository.updateStatusByHashes(scanHashes, 'ocr_complete');
                    Logger.info('ICR_OCR_PROCESSING', 'Updated scan statuses after OCR completion', {
                        scanCount: scanHashes.length,
    ...
    async processOcrByHashes(imageHashes) {
        try {
            ...
            // Validate minimum 20 labels requirement
            const labelCount = stitchedLabel.labelHashes?.length || 0;
            if (labelCount < 20) {
                Logger.error('ICR_OCR_BY_HASHES', 'Insufficient labels for OCR processing', {
                    labelCount,
                    minimumRequired: 20,
                    stitchedLabelId: stitchedLabel._id
                });
                throw new Error(`OCR requires minimum 20 labels stitched together. Found ${labelCount} labels. Please stitch more labels before running OCR.`);
            }
