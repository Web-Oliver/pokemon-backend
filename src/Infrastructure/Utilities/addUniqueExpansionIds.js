...
class UniqueExpansionIdGenerator {
  ...
  loadExpansionsList() {
    ...
    if (!FileUtils.fileExists(expansionListPath)) {
      throw new Error(`Expansion names list not found: ${expansionListPath}`);
    }

    const expansionData = FileUtils.readJsonFile(expansionListPath);
    ...
    FileUtils.writeJsonFile(expansionListPath, updatedExpansionData);
...
export default UniqueExpansionIdGenerator;
