...
class SearchService {
  ...
  async searchCards(query, filters = {}, options = {}) {
    ...
    try {
      ...
      let seenIds = new Set(); // FIXED: Use 'let' instead of 'const' for reassignment
      ...
      searchResults = allResults.filter(result => {
        ...
        // FIXED: Safe Set usage with error handling
        const hasNew = resultArray.some(id => {
          try {
            return !seenIds.has(id);
          } catch (err) {
            console.error('[FLEXSEARCH] Error in seenIds.has, reinitializing Set:', err);
            seenIds = new Set();
            return true;
          }
        });
        ...
        resultArray.forEach(id => {
          try {
            ...
            console.error('[FLEXSEARCH] Error in seenIds.add, reinitializing Set:', err);
            seenIds = new Set();
            seenIds.add(id);
          }
        });
      ...
      if (cardIds.length > 0) {
        ...
        cardIds.forEach(id => {
          ...
          if (card && orderedResults.length < limit) {
            // Enhanced set display name handling with multiple fallbacks
            let setDisplayName = 'Unknown Set';
            let fallbackSetName = true;

            // Priority 1: Use populated setId.setName
            if (card.setId?.setName) {
              setDisplayName = card.setId.setName;
              fallbackSetName = false;
            }
            // Priority 2: Use existing setDisplayName
            else if (card.setDisplayName) {
              setDisplayName = card.setDisplayName;
              fallbackSetName = false;
            }
            // Priority 3: Try uniqueSetId lookup
            else if (card.uniqueSetId) {
              // This would need to be resolved asynchronously in a real implementation
              // For now, mark as needing resolution
              setDisplayName = `Set ID: ${card.uniqueSetId}`;
              fallbackSetName = true;
            }

            card.setDisplayName = setDisplayName;
            card.fallbackSetName = fallbackSetName;
            // Ensure setName is also set for compatibility
            if (!card.setName) {
              card.setName = setDisplayName;
            }
