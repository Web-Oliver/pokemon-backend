...
class DbaExportService {
   ...
   * @param {string} publicPath - Path to public uploads folder
   * @returns {Promise<string>} - Path to created data folder
   */
  async copyImagesToDataFolder(dbaItems, publicPath = './public') {
      ...
      for (const item of dbaItems) {
        for (const originalPath of item.originalImagePaths) {
          try {
            ...
            const sourcePath = path.join(publicPath, originalPath);
  ...
  async generateDbaExport(items, options = {}) {
    ...
    try {
      ...
      const dataFolder = await this.copyImagesToDataFolder(dbaItems);
    ...
    try {
      ...
      for (const filename of selectedImageFiles) {
        ...
        if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
          ...
          console.warn(`[DBA EXPORT] Selected image not found: ${filename}`);
        }
      }
