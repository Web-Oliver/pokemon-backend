...
import SetEntity from '@/pokemon/sets/Set.js';
/**
 * Search Cache Class
 *
  ...
  invalidateByEntity(entityType, entityId = null) {
    ...
    switch (entityType) {
      ...
      case 'psa':
        ...
        break;
      case 'icr':
        // ICR operations should NOT be cached - removed inappropriate invalidation logic
        break;
      default:
        patterns.push('/api/unified-search');
    }
...
cacheManager.addWarmupStrategy('recentSets', async () => {
  ...
  try {
    const recentSets = await SetEntity.find({})
      .sort({ createdAt: -1 })
      .limit(10)
      .select('name _id');
...
export const getCacheStats = () => cacheManager.getMetrics();
export const invalidateCacheByEntity = (entityType, entityId) => cacheManager.invalidateByEntity(entityType, entityId);
export const invalidateCacheByPattern = (pattern) => cacheManager.invalidatePattern(pattern);
export const searchCacheMiddleware = enhancedCacheMiddleware;
