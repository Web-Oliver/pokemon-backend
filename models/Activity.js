...
const activitySchema = new mongoose.Schema({
  ...
  toJSON: {
    ...
    transform: function(doc, ret) {
      ...
      if (minutes < 1) ret.relativeTime = 'just now';
      else if (minutes < 60) ret.relativeTime = `${minutes} minute${minutes > 1 ? 's' : ''} ago`;
      else if (hours < 24) ret.relativeTime = `${hours} hour${hours > 1 ? 's' : ''} ago`;
      else if (days < 7) ret.relativeTime = `${days} day${days > 1 ? 's' : ''} ago`;
      else if (weeks < 4) ret.relativeTime = `${weeks} week${weeks > 1 ? 's' : ''} ago`;
      else if (months < 12) ret.relativeTime = `${months} month${months > 1 ? 's' : ''} ago`;
      else ret.relativeTime = new Date(ret.timestamp).toLocaleDateString();
