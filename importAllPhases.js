...
mongoose.connect(process.env.MONGO_URI).then(async () => {
  ...
  logAndCapture('INFO', `Deleted ${setsResult.deletedCount} sets`, { deletedCount: setsResult.deletedCount, collection: 'sets' });
  ...
  const remainingCardMarket = await mongoose.connection.db.collection('cardmarketreferenceproducts').countDocuments({});
  const remainingSealedProducts = await mongoose.connection.db.collection('sealedproducts').countDocuments({});


  logAndCapture('INFO', `Verification: ${remainingSets} sets, ${remainingCards} cards, ${remainingCardMarket} card market products, ${remainingSealedProducts} sealed products remaining`, {
    remainingSets,
    remainingCards,
    remainingCardMarket,
  ...
  logAndCapture('INFO', `Total sets checked: ${totalSetsChecked}`, { totalSetsChecked });
  if (setsWithZeroActualCards > 0) {
    logAndCapture('ERROR', `Sets with 0 actual cards in DB (but expected more): ${setsWithZeroActualCards}`, { count: setsWithZeroActualCards, type: 'zero_cards_error' });
  }
