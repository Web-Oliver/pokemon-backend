...
describe('Auctions PUT/DELETE operations', () => {
  ...
  describe('PUT /auctions/:id', () => {
    it('should update auction', async () => {
      const auction = await new Auction({ topText: 'Original', bottomText: 'Auction' }).save();

      const res = await request(app)
        .put(`/auctions/${auction._id}`)
        .send({ topText: 'Updated' });
    ...
    it('should return 404 for non-existent auction', async () => {
      const res = await request(app)
        .put(`/auctions/${new mongoose.Types.ObjectId()}`)
        .send({ topText: 'Updated' });
  ...
  describe('DELETE /auctions/:id', () => {
    it('should delete auction', async () => {
      const auction = await new Auction({ topText: 'Delete', bottomText: 'Me' }).save();
